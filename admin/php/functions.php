...
function generate_schedule($sy,$year_id,$room){
	...
	if($num>0){
		...
		while($row=mysql_fetch_assoc($yls)){
			...
			$sel = mysql_query("SELECT * FROM schedule as sched join year_level_subject as yls on sched.yls_id=yls.yls_id where sched.days='$mwf' and sched.schoolyear='$sy' and yls.year_id='$year_id' ");
			$num = mysql_num_rows($sel);
			if($num<7){
				$timehr = $time[$x];
				$ins = mysql_query("INSERT INTO schedule (timehr,days,schoolyear,yls_id,room) VALUEs ('$timehr','$mwf','$sy','$yls_id','$room') ");
				$x +=1;
			}
			else{
				$timehr = $time[$x];
				$ins = mysql_query("INSERT INTO schedule (timehr,days,schoolyear,yls_id,room) VALUEs ('$timehr','$tth','$sy','$yls_id','$room') ");
				$x +=1;
			}	
